...
/*
    NOTE: BloodBox
          Changed made throughout this file
*/
...
#define DEFAULT_CONFIG_FILE "/bloodbox.conf"
#elif defined(MACOSX)
#define DEFAULT_CONFIG_FILE "/Library/Preferences/BloodBox Preferences"
#else /*linux freebsd*/
#define DEFAULT_CONFIG_FILE "/.bloodboxrc"
#endif
...
void GFX_SetTitle(Bit32s cycles,Bits frameskip,bool paused){
	...
	if(CPU_CycleAutoAdjust) {
		sprintf(title,"(BloodBox) DOSBox %s, Cpu speed: max %3d%% cycles, Frameskip %2d, Program: %8s",VERSION,internal_cycles,internal_frameskip,RunningProgram);
	} else {
		sprintf(title,"(BloodBox) DOSBox %s, Cpu speed: %8d cycles, Frameskip %2d, Program: %8s",VERSION,internal_cycles,internal_frameskip,RunningProgram);
	}
...
static void GUI_StartUp(Section * sec) {
   ...
   if(sdl.desktop.want_type==SCREEN_OPENGL){ /* OPENGL is requested */
	...
	GFX_Stop();
	SDL_WM_SetCaption("BloodBox, based on DOSBox",VERSION);
...
static void eraseconfigfile() {
	FILE* f = fopen("bloodbox.conf","r");
	if(f) {
		fclose(f);
		show_warning("Warning: bloodbox.conf exists in current working directory.\nThis will override the configuration file at runtime.\n");
	}
	std::string path,file;
...
static void erasemapperfile() {
	FILE* g = fopen("bloodbox.conf","r");
	if(g) {
		fclose(g);
		show_warning("Warning: bloodbox.conf exists in current working directory.\nKeymapping might not be properly reset.\n"
		             "Please reset configuration as well and delete the bloodbox.conf.\n");
	}
...
int main(int argc, char* argv[]) {
	try {
		...
		if (control->cmdline->FindExist("-noconsole")) {
			...
			SetConsoleTitle("(BloodBox) DOSBox Status Window");
		}
#endif  //defined(WIN32) && !(C_DEBUG)
		if (control->cmdline->FindExist("-version") ||
		    control->cmdline->FindExist("--version") ) {
            printf("\nBloodBox, based on\n");
			printf("DOSBox version %s, copyright 2002-2010 DOSBox Team.\n\n",VERSION);
	...
	LOG_MSG("BloodBox, based on DOSBox version %s",VERSION);
	...
	if(!parsed_anyconfigfile) {
        ...
        uint32_t Ok = 1;
        if (!parsed_anyconfigfile) {
            LOG_MSG("ERROR: Could not find bloodbox.conf\n");
            Ok = 0;
        }
