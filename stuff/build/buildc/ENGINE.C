...
#include "build.h"
#include "pragmas.h"
volatile long stereomode = 0, whiteband, blackband; //08/20/2003: hack to make it compile with new VES2.H
#include "ves2.h"
...
volatile long visualpage, activepage;
	...
	*hitx = hitscangoalx; *hity = hitscangoaly;
	...
	do
	{
		...
		if (sec->ceilingstat&2)
		{
			...
			i = msqrtasm(dax*dax+day*day); if (i == 0) continue;
...
dorotatesprite (long sx, long sy, long z, short a, short picnum, signed char dashade, char dapalnum, char dastat, long cx1, long cy1, long cx2, long cy2)
{
	...
	long xv, yv, xv2, yv2, qlinemode, y1ve[4], y2ve[4], u4, d4;
	char bad;
	...
	if (((vidoption != 1) || (buffermode != 0)) && (dastat&8))
		 permanentupdate = 1;
	...
	for(s=0,sec=&sector[s];s<numsectors;s++,sec++)
		if (show2dsector[s>>3]&pow2char[s&7])
		{
			...
			else
			{
				...
				i = msqrtasm(ox*ox+oy*oy); if (i == 0) continue;
				i = 1048576/i;
...
grouscan (long dax1, long dax2, long sectnum, char dastat)
{
	...
	for(x=dax1;x<=dax2;x++)
	{
		...
		if (y1 <= y2)
		{
			...
			while (nptr1 <= mptr1)
			{
				*mptr1-- = j + (getpalookup((long)mulscale24(krecip(m1),globvis),globalshade)<<8);
				m1 -= l;
			}
			while (nptr2 >= mptr2)
			{
				*mptr2++ = j + (getpalookup((long)mulscale24(krecip(m2),globvis),globalshade)<<8);
				m2 += l;
			}
			...
			slopevlin(ylookup[y2]+x+frameoffset,krecip(asm3>>3),(long)nptr2,y2-y1+1,globalx1,globaly1);
