...
bool DOS_Execute(char * name,PhysPt block_pt,Bit8u flags) {
    ...
    /* NOTE: BloodBox */
    int loading_dos32a;
    if (!strcmp(name, "dos32a.exe")) {
        printf("Executing dos32a.exe\n");
        loading_dos32a = 1;
    } else {
        loading_dos32a = 0;
    }
...
//			if (readsize!=imagesize) LOG(LOG_EXEC,LOG_NORMAL)("Illegal header");
    ...
    if (loading_dos32a)
    {
        ...
        */
        Bit16u _Seg = loadseg + head.initCS;
        Bit16u _Ofs = head.initIP + 0x1bf;

        real_writeb(_Seg, _Ofs++, 0x31);
        real_writeb(_Seg, _Ofs++, 0xc0);
        real_writeb(_Seg, _Ofs++, 0xcd);
        real_writeb(_Seg, _Ofs++, 0xbd);
        real_writeb(_Seg, _Ofs++, 0x90);
        real_writeb(_Seg, _Ofs  , 0x90);
    }
    /*--------------------------------------*/
